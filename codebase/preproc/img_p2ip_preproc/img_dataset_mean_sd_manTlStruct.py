...
def calc_mean_sd_whole_img_dataset(root_path='./', spec_type='human', img_resoln=256, dbl_combi_flg=False, stage='test'):
    ...
    if stage == "fit" or stage is None:  # for training
        ...
        for step, (images, _) in enumerate(train_loader):
            if(step % 500 == 0): print('step = ' +str(step) + ' completed out of ' + str(int((len(pp_pair_lst_lsts_train) / batch_size))))
        ...
        var = 0.0
        for step, (images, _) in enumerate(train_loader):
            if(step % 500 == 0): print('\n ##################### \n step = ' +str(step) + ' completed out of ' + str(int((len(pp_pair_lst_lsts_train)) / batch_size)) + '  \n ################\n')
            batch_samples = images.size(0)
            images = images.view(batch_samples, images.size(1), -1)
...
if __name__ == '__main__':
    ...
    img_resoln_lst = [400, 800]
    # dbl_combi_flg_lst = [False, True]
    dbl_combi_flg_lst = [True]
    stage = 'fit'  # fit, test
    ...
    mean_std_train_df.to_csv(os.path.join(root_path, 'dataset/preproc_data_tl_feat_to_img/train_test_list_dump', 'mean_std_train_manTlStruct.csv'), index=False)
