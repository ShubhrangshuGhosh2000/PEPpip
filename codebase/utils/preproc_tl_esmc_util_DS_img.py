...
def extract_feat_from_esmc(prot_id_lst, seq_lst, esmc_model_path='./', esmc_model_name = 'esmc_600m', spec_type = 'human'):
    ...
    extract_feat_from_preloaded_esmc(prot_id_lst, seq_lst, model, spec_type, device='gpu')


# use preloaded esmc model to extract the features for a given list of sequences 
def extract_feat_from_preloaded_esmc(prot_id_lst, seq_lst, model, spec_type, device='gpu'):
    ...
    spec_result_dir = os.path.join(root_path, 'dataset/preproc_data_DS/tl_esmc_2d_feat_dict_dump_img', spec_type)
    ...
    for itr in range(latest_pkl_file_index + 1, tot_no_of_parts):
        ...
        # squeeze z to 2d-array (e.g. from (1, 128, 1152) to (128, 1152))
        z = z.squeeze()
        ...
        if(device == 'cpu'):
            ...
            return extract_feat_from_preloaded_esmc(prot_id_lst, seq_lst, model, spec_type, device='gpu')
    ...
    if(cuda_error):
        ...
        return extract_feat_from_preloaded_esmc(prot_id_lst, seq_lst, model, spec_type, device='cpu')
