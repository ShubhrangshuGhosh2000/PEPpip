...
def loadDscriptData_human_full(resultsFolderName):
    ...
    saves = []
    predFNames = []
    trainSets = PPIPUtils.parseTSV(featureFolder+'DS_train_pairs_human_seq_len400.tsv','string')
    # trainSets = PPIPUtils.parseTSV(featureFolder+'human_full.tsv','string')
    trainSets = [np.asarray(trainSets)]
    saves = [resultsFolderName+'DS_human_len400.out']
...
def loadDscriptData(resultsFolderName, spec_type = 'human'):
    ...
    saves = []
    predFNames = []
    testSets = PPIPUtils.parseTSV(featureFolder + f'DS_test_pairs_{spec_type}_seq_len400.tsv','string')
