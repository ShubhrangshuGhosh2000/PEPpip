...
def extract_feat_from_preloaded_protTrans(seq_lst, model, tokenizer, device='gpu'):
    ...
    temp_result_dir = os.path.join(root_path, 'temp_result')  # for the iteration-wise intermediate pkl file saving purpose
    temp_per_prot_emb_result_dir = os.path.join(root_path, 'temp_per_prot_emb_result')  # for the iteration-wise intermediate per protein embedding list saving purpose
    ...
    for itr in range(latest_pkl_file_index + 1, tot_no_of_parts):
        ...
        if(itr % 100 == 0): print('######### completed ' + str(itr) + '-th iteration out of ' + str(tot_no_of_parts-1))
    ...
    for itr in range(loop_start_index, tot_no_of_parts):
        if((itr % 1 == 0) or (itr in [(tot_no_of_parts - 1)])):
            ...
            if(itr % 200== 0) : print('performing pooling column-wise :: itr = ' + str(itr))
