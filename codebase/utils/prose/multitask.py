...
# from utils.prose.prose_utils import get_project_root
...
class ProSEMT(nn.Module):
    ...
    @staticmethod
    def load_pretrained(prose_model_name_with_path):
        # if path is None or path == 'prose_mt':
        #     root = get_project_root()
        #     path = os.path.join(root, 'saved_models', 'prose_mt_3x1024.sav')
        path = prose_model_name_with_path

        from utils.prose.lstm import SkipLSTM
...
class BilinearContactMap(nn.Module):
    """
    Predicts interaction maps as sigmoid(z_i W z_j + b)
    """
