...
def extract_feat_from_prose(prot_id_lst, seq_lst, prose_model_path='./', prose_model_name = 'prose_mt_3x1024', docking_version = '5_5'):
    ...
    extract_feat_from_preloaded_prose(prot_id_lst, seq_lst, model, docking_version, device='gpu')


# use preloaded prose model(s) to extract the features for a given list of sequences 
def extract_feat_from_preloaded_prose(prot_id_lst, seq_lst, model, docking_version, device='gpu'):
    ...
    spec_result_dir = os.path.join(root_path, 'dataset/preproc_data_docking_BM_' + str(docking_version), 'dock_tl_struct_2d_feat_dict_dump_img')
    ...
    for itr in range(latest_pkl_file_index + 1, tot_no_of_parts):
        ...
        if(itr % 50 == 0): print('###### docking_version: ' + docking_version + ' ### completed ' + str(itr) + '-th iteration out of ' + str(tot_no_of_parts-1))
        ...
        if(device == 'cpu'):
            ...
            return extract_feat_from_preloaded_prose(prot_id_lst, seq_lst, model, docking_version, device='gpu')
    ...
    if(cuda_error):
        ...
        return extract_feat_from_preloaded_prose(prot_id_lst, seq_lst, model, docking_version, device='cpu')
