...
class CDAMViT(nn.Module):
    ...
    def forward(self, x):
        ...
        if not self.training and torch.is_grad_enabled():  # In evaluation mode and gradient tracking is enabled
            ...
            if final_layer.norm_first:  # For CDAM calculation, final_layer.norm_first should be always True
            ...
            else:  # For CDAM calculation, final_layer.norm_first should never be False i.e. this else-block should never be executed
            ...
            # For torch.autograd.grad() to work correctly:
            # 1. Input tensors should have requires_grad=True.
            # 2. Output tensors will automatically have gradients tracked if they're computed from tensors with requires_grad=True
            # 3. The most crucial requirement for torch.autograd.grad() is that inputs and outputs must be connected in the same computation graph. There 
            # must be a direct path from outputs to inputs in the computational graph created during the forward pass.
            # 4. Only floating-point and complex tensors can require gradients.
            ...
            # print(f'###########################\n grads: {grads}\n ##################')
            token_importance = (final_block_input_norm * grads).sum(dim=-1)  # [B, N+1]
            # print(f'###########################\n token_importance: {token_importance}\n ##################')

            # # Option-1
            # # ## Use self.patch_stat_2dTensor on token_importance except for the class token index (as we are only interested 
            # # ## to obtain the interaction map in this case).
            # token_importance_norm = (token_importance[:, 1:] - self.patch_stat_2dTensor[:, :, 0])/ (self.patch_stat_2dTensor[:, :, 1] + 1e-8)  #  (B, N)
            # token_importance = torch.cat((token_importance[:, 0:1], token_importance_norm), dim=1)  # concat column-wise : shape: (B, N+1)
            ...
            # # Option-2 (Preferred than Option -1 above, specially for tp-total-R-CSGB case)
            # # Use self.patch_stat_2dTensor on patch_attn
            # patch_attn = (patch_attn[:, :] - self.patch_stat_2dTensor[:, :, 0])/ (self.patch_stat_2dTensor[:, :, 1] + 1e-8)
            
            # Min-max normalization to range [0.0, 1.0] for each sample in batch independently
            patch_attn_min = patch_attn.min(dim=1, keepdim=True)[0]  # [B, 1]
            patch_attn_max = patch_attn.max(dim=1, keepdim=True)[0]  # [B, 1]
            patch_attn = (patch_attn - patch_attn_min) / (patch_attn_max - patch_attn_min + 1e-8)  # [B, N]
        ...
        # End of if-else block: if not self.training and torch.is_grad_enabled():
