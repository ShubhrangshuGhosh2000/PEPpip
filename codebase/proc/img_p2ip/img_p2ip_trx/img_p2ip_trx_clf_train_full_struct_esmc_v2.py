...
class ImgP2ipTrx(L.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.1, patience=5)  # for train_loss
        # scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', factor=0.1, patience=5)  # for train_aupr

        return {
            'optimizer': optimizer,
            'lr_scheduler': scheduler,
            'monitor': 'train_loss' # train_loss, train_aupr
        }
...
def train_model(root_path='./', result_dir='./', model_name='CDAMViT', save_name='CDAMViT', config = None, prv_ckpt_path=None):
    ...
    checkpoint_callback = L.pytorch.callbacks.ModelCheckpoint(
                        ...
                        , filename="ImgP2ipTrx" + "-epoch{epoch:02d}-train_aupr{train_aupr:.2f}-last"
                        , auto_insert_metric_name=False
                        , save_top_k=1
                        , save_last=True  # always save the last checkpoint as 'last.ckpt'
                        , monitor='train_loss', mode='min'
                        # , monitor='train_aupr', mode='max' 
...
if __name__ == '__main__':
    ...
    prv_ckpt_path = None
    prv_ckpt_path = os.path.join(root_path, 'dataset/proc_data_tl_feat_to_img/img_p2ip_trx/train/CDAMViT_tlStructEsmc_r400p16_fullTrain')
    start(root_path=root_path, model_name='CDAMViT', save_name='CDAMViT_tlStructEsmc_r400p16_fullTrain', prv_ckpt_path=prv_ckpt_path)
