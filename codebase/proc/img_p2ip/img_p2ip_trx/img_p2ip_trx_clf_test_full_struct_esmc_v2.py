...
from proc.img_p2ip.img_p2ip_trx.img_p2ip_trx_clf_train_full_struct_esmc_v2 import ImgP2ipTrx
...
def prepare_test_data(root_path='./', model=None, spec_type='ecoli'):
    ...
    test_data_module = ImgP2ipCustomDataModule(root_path=root_path
                                               ...
                                               , workers=os.cpu_count() - 2  # model.hparams.config['num_workers']
...
def test_model(root_path='./', model_path='./', partial_model_name = 'ImgP2ipTrx', spec_type='ecoli'):
    ...
    for pred_idx in range(tot_no_predictions):
        ...
        # be 0 or 1.
        full_pred_prob_arr = out.numpy()  # out.cpu().numpy()
    ...
    # Free-up GPU memory -Start
    del model  # Delete the model
    gc.collect()  # Collect garbage
    torch.cuda.empty_cache()  # Empty CUDA cache
    # Free-up GPU memory -End

    print('#### inside the test_model() method - End')
    # return con_pred_attn_2d_tensor, con_test_label_1d_tensor
