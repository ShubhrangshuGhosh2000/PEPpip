...
# def start_without_tuning(root_path='./', model_name = 'CDAMViT', save_name=None, prv_ckpt_path=None):
...
#         config["optimizer_name"] = 'Adam'  # Adam, SGD 
...
#     # img_p2ip_trx_clf_train_final.start(root_path)

#     # perform the testing 
#     # print('\n# ############################# performing the testing #############################\n')
#     # img_p2ip_trx_clf_test.start(root_path = root_path)
...
def ray_hparam_tuning(config = None, root_path='./', result_dir='./', model_name='CDAMViT', cpus_per_trial=15, gpus_per_trial=1, num_samples=10, scheduler_name='ASHA'):
    ...
    # start_without_tuning(root_path=root_path, model_name='CDAMViT', save_name='CDAMViT_temp', prv_ckpt_path=None)
    ...
    analysis = ray.tune.run(
        ...
        checkpoint_score_attr='max-val_aupr',  # Specifies by which attribute to rank the best checkpoint.
...
def start(root_path='./', model_name = 'CDAMViT'):
    ...
    config = {}
    if(model_name == 'CDAMViT'):
        config["patch_size"] = ray.tune.grid_search([16])  # 8, 16
        config["num_heads"] = ray.tune.choice([8, 12])
        config["hidden_dim"] = ray.tune.choice([768])  # 512, 768
        config["batch_size"] =  ray.tune.grid_search([24])  # 16, 24

        config["num_classes"] = 2  # hparam for the model architecture
        # config["act_fn_name"] = 'relu'  # tanh, relu, leakyrelu, gelu
        config["optimizer_name"] = ray.tune.grid_search(['Adam'])  # Adam, SGD (FOR CDAMViT BOTH NEEDS TO BE CHECKED)
        ...
        # config["num_workers"] = os.cpu_count() - 5  # for the DataModule creation
        config["num_workers"] = 2  # for the DataModule creation

    # depending upon the sheduler, set 'lr' value in the config
    if(scheduler_name == 'ASHA'):
        config["lr"] = ray.tune.loguniform(1e-5, 1e-3)  # for ASHA Scheduler
    ...
    cpus_per_trial = 8  # cpus_per_trial=1 implies the no. of concurrent trials at a time = the no of CPU (cores) in the machine
...
if __name__ == '__main__':
    ...
    # start(root_path=root_path, model_name='CDAMViT', save_name='CDAMViT_expt_r400', prv_ckpt_path=prv_ckpt_path)
