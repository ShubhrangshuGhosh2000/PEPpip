...
class DockImgP2ipCustomDataset(Dataset):
    # constuctor
    def __init__(self, root_path='./', spec_type='human', docking_version='4_0', partial_twoD_prot_feat_dict=None, man_2d_feat_dict=None
    ...
    def __getitem__(self, idx):
        ...
        # accommodate struct embeddings from Prose
        prot1_struct_2dArr_file_nm_loc = os.path.join(self.root_path, 'dataset/preproc_data_docking_BM_' + str(self.docking_version), 'dock_tl_struct_2d_feat_dict_dump_img', f"prot_id_{prot1_id}.pkl")
        prot1_struct_2dArr = joblib.load(prot1_struct_2dArr_file_nm_loc)
        prot2_struct_2dArr_file_nm_loc = os.path.join(self.root_path, 'dataset/preproc_data_docking_BM_' + str(self.docking_version), 'dock_tl_struct_2d_feat_dict_dump_img', f"prot_id_{prot2_id}.pkl")
        prot2_struct_2dArr = joblib.load(prot2_struct_2dArr_file_nm_loc)
        # make each 2dArr in the shape (800 x 3000)
        prot1_struct_2dArr_sameDim = np.zeros((800, 3000))
        if(prot1_struct_2dArr.shape[0] > 800):  # adjust the number of residues of prot1 upto a maximum of 800
            prot1_struct_2dArr = prot1_struct_2dArr[:800, :]
        prot1_struct_2dArr_sameDim[:prot1_struct_2dArr.shape[0], :prot1_struct_2dArr.shape[1]] = prot1_struct_2dArr
        prot2_struct_2dArr_sameDim = np.zeros((800, 3000))
        if(prot2_struct_2dArr.shape[0] > 800):  # adjust the number of residues of prot1 upto a maximum of 800
            prot2_struct_2dArr = prot2_struct_2dArr[:800, :]
        prot2_struct_2dArr_sameDim[:prot2_struct_2dArr.shape[0], :prot2_struct_2dArr.shape[1]] = prot2_struct_2dArr
        prot12_mixed_struct_2dArr = np.matmul(prot1_struct_2dArr_sameDim, prot2_struct_2dArr_sameDim.transpose())
        prot12_mixed_struct_2dArr = prot12_mixed_struct_2dArr[:self.img_resoln, :self.img_resoln]

        # accommodate sequence based Tl-embeddings from ProtTrans
        ...
        # accommodate sequence based 2D manually curated features
        ...
        prot_prot_3dArr = np.dstack((prot12_mixed_struct_2dArr, prot12_mixed_man_feat_2dArr, modified_prot12_mixed_2dArr))
    ...
    def get_item_with_orig_input(self, idx):
        ...
        prot1_2dArr_protLevel[:prot1_2dArr_protLevel_orig.shape[0], :prot1_2dArr_protLevel_orig.shape[1]] = prot1_2dArr_protLevel_orig
        prot2_2dArr_protLevel = np.zeros((800, 1024))
        if(prot2_2dArr_protLevel_orig.shape[0] > 800):  # adjust the number of residues of prot1 upto a maximum of 800
            prot2_2dArr_protLevel_orig = prot2_2dArr_protLevel_orig[:800, :]
        prot2_2dArr_protLevel[:prot2_2dArr_protLevel_orig.shape[0], :prot2_2dArr_protLevel_orig.shape[1]] = prot2_2dArr_protLevel_orig
        prot12_mixed_2dArr = np.matmul(prot1_2dArr_protLevel, prot2_2dArr_protLevel.transpose())
        # print('prot12_mixed_2dArr.shape: \n' + str(prot12_mixed_2dArr.shape))
