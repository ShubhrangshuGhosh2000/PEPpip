...
def calc_candidatePPI_p_val(root_path='./', model_path='./', docking_version='4_0', attr_mode='prose', min_max_mode='R', no_random_shuffle=500
    ...
    tp_df = gt_vs_pred_emd_df[ (gt_vs_pred_emd_df['tp_fn'] == 'tp') & (gt_vs_pred_emd_df['attr_mode'] == attr_mode) & (gt_vs_pred_emd_df['min_max_mode'] == min_max_mode) & (gt_vs_pred_emd_df['pcmp_mode'] == pcmp_mode)]
    tp_df = tp_df.reset_index(drop=True)
    fn_df = gt_vs_pred_emd_df[ (gt_vs_pred_emd_df['tp_fn'] == 'fn') & (gt_vs_pred_emd_df['attr_mode'] == attr_mode) & (gt_vs_pred_emd_df['min_max_mode'] == min_max_mode) & (gt_vs_pred_emd_df['pcmp_mode'] == pcmp_mode)]
    ...
    for index, row in tp_df.iterrows():
        ...
        tp_pred_contact_map_path = os.path.join(tp_pred_contact_map_proc_loc, f"pred_contact_map_{protein_name}_{chain_1_name}_{chain_2_name}_attrMode_{attr_mode}_minMaxMode_{min_max_mode}_pcmp_mode_{pcmp_mode}.pkl")
        ...
        print(f'consider_full: {consider_full} :: attr_mode: {attr_mode} :: min_max_mode: {min_max_mode} :: pcmp_mode: {pcmp_mode}')
        ...
        print('randomly shuffle pred_contact_map (a 2D numpy array) along a specified axis multiple times and\n \
                for each shuffling, calculate the EMD between shuffled version and gt_contact_map')
        emd_for_shuffled_lst = DockUtils.shuffle_predContactMap_and_calc_emd(pred_contact_map, gt_contact_map, axis=1, no_random_shuffle=no_random_shuffle, seed=456
    ...
    if(consider_fn):
        ...
        for index, row in fn_df.iterrows():
            ...
            # retrieve the pred_contact_map
            print('retrieving the pred_contact_map...')
            fn_pred_contact_map_path = os.path.join(fn_pred_contact_map_proc_loc, f"pred_contact_map_{protein_name}_{chain_1_name}_{chain_2_name}_attrMode_{attr_mode}_minMaxMode_{min_max_mode}_pcmp_mode_{pcmp_mode}.pkl")
    ...
    candidatePPI_p_val_lst_loc = os.path.join(tTest_result_dir, f"candPPI_pVal_lst_considerFN_{consider_fn}_considerFull_{consider_full}_attrMode_{attr_mode}_minMaxMode_{min_max_mode}_pcmp_mode_{pcmp_mode}.pkl")
...
def perform_one_tailed_t_test(root_path='./', model_path='./', docking_version='4_0', consider_fn=False, consider_full=False, attr_mode='prose', min_max_mode='R', pcmp_mode='SCGB'):
    ...
    one_tailed_t_test_res_dict = {'consider_fn': consider_fn, 'consider_full': consider_full, 'attrMode': attr_mode, 'minMaxMode': min_max_mode, 'pcmp_mode': pcmp_mode
                                  , 't-statistic': result.statistic, 'p-value': result.pvalue, 'alpha': alpha, 'conclusion': conclusion}
    one_tailed_t_test_res_dict_loc = os.path.join(tTest_result_dir, f"t_test_res_dict_considerFN_{consider_fn}_considerFull_{consider_full}_attrMode_{attr_mode}_minMaxMode_{min_max_mode}_pcmp_mode_{pcmp_mode}.pkl")
...
def gen_violin_plt_of_pVal_for_tlStructEsmc(root_path='./', docking_version='4_0'):
    ...
    print('\n########## docking_version: ' + str(docking_version))
    test_tag = 'ResNet_tlStructEsmc_r400n18DnoWS'
    ...
    consider_fn=False
    candidatePPI_p_val_lst_fn_False_loc = os.path.join(tTest_result_dir, f"candPPI_pVal_lst_considerFN_{consider_fn}_considerFull_{consider_full}_attrMode_{attr_mode}_minMaxMode_{min_max_mode}_pcmp_mode_{pcmp_mode}.pkl")
    # load candidatePPI_p_val_lst from pkl file
    ...
    consider_fn=True
    candidatePPI_p_val_lst_fn_True_loc = os.path.join(tTest_result_dir, f"candPPI_pVal_lst_considerFN_{consider_fn}_considerFull_{consider_full}_attrMode_{attr_mode}_minMaxMode_{min_max_mode}_pcmp_mode_{pcmp_mode}.pkl")
    # load candidatePPI_p_val_lst from pkl file
...
if __name__ == '__main__':
    ...
    attr_mode_lst = ['total']  # 'total', 'prot_trans', 'esmc', 'prose'
                ...
                for attr_mode in attr_mode_lst:
                    ...
                    for min_max_mode in min_max_mode_lst:
                        ...
                        for pcmp_mode in pcmp_mode_lst:
                            ...
                            calc_candidatePPI_p_val(root_path=root_path, model_path=model_path, docking_version=docking_version, attr_mode=attr_mode, min_max_mode=min_max_mode
                            ...
                            one_tailed_t_test_res_dict = perform_one_tailed_t_test(root_path=root_path, model_path=model_path, docking_version=docking_version
                                                                               , attr_mode=attr_mode, min_max_mode=min_max_mode, consider_fn=consider_fn, consider_full=consider_full, pcmp_mode=pcmp_mode)
