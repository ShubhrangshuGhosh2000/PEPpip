...
def create_predicted_contact_map(root_path='./', model_path='./', docking_version='4_0', min_max_mode='R', attr_mode='total', pcmp_mode='SCGB'):
    ...
    for itr, row in tp_df.iterrows():
        ...
        if(attr_mode == 'total'):
            ...
            # retrieve 'non_zero_pxl_attr_lst_lst' from postproc_res_dict
            pxl_attr_lst = postproc_res_dict['non_zero_pxl_attr_lst_lst']
        # end of if-else block: if(attr_mode == 'total'):
...
def create_pred_attr_map_2d(non_zero_pxl_indx_lst_lst, pxl_attr_lst, attr_mode):
    ...
    # Iterate over non_zero_pxl_indx_lst_lst and populate pred_attr_map_2d based on non_zero_pxl_attr_lst
...
def calculate_emd_betwn_gt_and_pred_contact_maps(root_path='./', model_path='./', docking_version='4_0'
    ...
    for index, row in dock_test_df.iterrows():
        ...
        pcmp_mode = pred_contact_map_name.split('.')[0].split('_')[-1]  # f"pred_contact_map_{protein_name}_{chain_1_name}_{chain_2_name}_attrMode_{attr_mode}_minMaxMode_{min_max_mode}_pcmp_mode_{pcmp_mode}.pkl"
...
def generate_gt_contact_heatmap_plot(data_array_2d, title='Heatmap Plot with Colormap', row='prot_1_id', col='prot_2_id', save_path='heatmap_plot.png'):
    ...
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    # Show the plot (optional)
    # plt.show()create_pred_attr_map_2d
    # Close the plot
    plt.close()
