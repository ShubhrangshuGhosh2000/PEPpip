...
def generate_single_violin_plot(data_frame=None, save_plot_fl_nm_loc='./'):
    ...
    ax.set_title("Violin Plot of abs difference for 'manPlm' and 'manPlmStruct'")
    ax.set_xlabel("")
    ax.set_ylabel("abs difference")
    # ax.legend()

    # # violin-plot using only matplotlib package
    # fig = plt.figure(figsize=(6, 6))
    # gs = fig.add_gridspec(nrows=1,ncols=2)

    # ax = fig.add_subplot(gs[0, 0])
    # plt.violinplot(dataset=data_frame[data_frame['Category'] == 'manPlm']['Values'].tolist()
    #                , showmedians=True, showextrema=True)
    # ax.set_xticks([])
    # ax.set_xlabel("manPlm")

    # ax = fig.add_subplot(gs[0, 1])
    # plt.violinplot(dataset=data_frame[data_frame['Category'] == 'manPlmStruct']['Values'].tolist()
    #                , showmedians=True, showextrema=True)
    # ax.set_xticks([])
    # ax.set_xlabel("manPlmStruct")
...
def gen_violin_plt_of_absDiff_for_manTl_manTlStruct(root_path='./', docking_version='5_5'):
    ...
    # find the absolute difference correponding to manTl features
    print('finding the absolute difference correponding to manTl features')
    test_tag = 'ResNet_manTl_r400n18D'
    xai_result_dir = os.path.join(root_path, f'dataset/proc_data_tl_feat_to_img/img_p2ip_cnn/xai_dock_{docking_version}/{test_tag}')
    emd_result_dir = os.path.join(xai_result_dir, 'emd_result')
    emd_df = pd.read_csv(os.path.join(emd_result_dir, f'gt_vs_pred_emd_full.csv'))
    abs_diff_lst_manTl = emd_df['abs_diff'].tolist()
    # perform an usual min-max normalization
    min_val, max_val = min(abs_diff_lst_manTl), max(abs_diff_lst_manTl)
    norm_abs_diff_lst_manTl = [(float(abs_diff)-min_val)/(max_val-min_val) for abs_diff in abs_diff_lst_manTl]
    # norm_abs_diff_lst_manTl = abs_diff_lst_manTl  # ################ TEMP CODE

    # find the absolute difference correponding to manTlStruct features
    print('finding the absolute difference correponding to manTlStruct features')
    test_tag = 'ResNet_manTlStruct_r400n18D'
    xai_result_dir = os.path.join(root_path, f'dataset/proc_data_tl_feat_to_img/img_p2ip_cnn/xai_dock_{docking_version}/{test_tag}')
    emd_result_dir = os.path.join(xai_result_dir, 'emd_result')
    emd_df = pd.read_csv(os.path.join(emd_result_dir, f'gt_vs_pred_emd_full.csv'))
    abs_diff_lst_manTlStruct = emd_df['abs_diff'].tolist()
    # perform an usual min-max normalization
    min_val, max_val = min(abs_diff_lst_manTlStruct), max(abs_diff_lst_manTlStruct)
    norm_abs_diff_lst_manTlStruct = [(float(abs_diff)-min_val)/(max_val-min_val) for abs_diff in abs_diff_lst_manTlStruct]
    # norm_abs_diff_lst_manTlStruct = abs_diff_lst_manTlStruct  # ################ TEMP CODE

    # create dataframe required for the violin plot generation
    manTl_manTlStruct_violin_plt_df = pd.DataFrame({'Category': ['manPlm'] * len(norm_abs_diff_lst_manTl) + ['manPlmStruct'] * len(norm_abs_diff_lst_manTlStruct)
                        , 'Values': norm_abs_diff_lst_manTl + norm_abs_diff_lst_manTlStruct})
    # specify location to save the violin plot
    manTl_manTlStruct_violin_plt_save_loc = os.path.join(common_plots_dir, f'violin_plt_absDiff_manTl_manTlStruct.png')
    ...
    print('\n #############################\n inside gen_violin_plt_of_absDiff_for_manTl_manTlStruct() method - End\n')
