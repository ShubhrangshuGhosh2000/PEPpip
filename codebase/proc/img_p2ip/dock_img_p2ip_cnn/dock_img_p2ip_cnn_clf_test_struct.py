...
from proc.img_p2ip.dock_img_p2ip_datamodule_struct import DockImgP2ipCustomDataModule
from proc.img_p2ip.img_p2ip_cnn.img_p2ip_cnn_clf_train_struct import ImgP2ipCnn
...
def create_len_limited_test_result(root_path='./', limiting_len=400, model_path='./', docking_version='4_0'):
    ...
    # also, read the dock_test_lenLimit_{limiting_len}.tsv in a pandas dataframe
    ...
    # next, iterate through dock_test_lenLimit_df and filter pred_result_df as per 'prot1_id' and 'prot2_id' values of each row in dock_test_lenLimit_df
    # Creating an empty dataframe pred_result_lenLimit_df
    pred_result_lenLimit_df = pd.DataFrame(columns=pred_result_df.columns)

    # Iterating through each row of dock_test_lenLimit_df
    for index, row in dock_test_lenLimit_df.iterrows():
        # Filtering rows from pred_result_df based on prot1_id and prot2_id values
        subset_df = pred_result_df[(pred_result_df['prot1_id'] == row['prot1_id']) & (pred_result_df['prot2_id'] == row['prot2_id'])]
        # Appending the subset to pred_result_lenLimit_df
        pred_result_lenLimit_df = pd.concat([pred_result_lenLimit_df, subset_df], ignore_index=True)
    # end of for loop
    # save pred_result_lenLimit_df
    pred_result_lenLimit_file_nm_with_loc = os.path.join(root_path, f'dataset/proc_data_tl_feat_to_img/img_p2ip_cnn/test_dock_{docking_version}/{test_tag}', f'pred_res_lenLimit_{limiting_len}.csv')
    pred_result_lenLimit_df.to_csv(pred_result_lenLimit_file_nm_with_loc, index=False)
    print('\n#### inside the create_len_limited_test_result() method - End')
...
if __name__ == '__main__':
    ...
    for docking_version in docking_version_lst:
        ...
        create_len_limited_test_result(root_path=root_path, limiting_len=400, model_path=model_path, docking_version=docking_version)
