...
from proc.img_p2ip.dock_img_p2ip_datamodule_struct_esmc_v2 import DockImgP2ipCustomDataModule
...
def calc_attribution(root_path='./', model_path='./', partial_model_name = 'ImgP2ipCnn', docking_version='5_5', device_type='cpu'):
    ...
    for index, row in pred_res_df.iterrows():
        ...
        model.zero_grad()
        # Note that, 'target' could always be set to 1 irrepective of the model prediction result to calculate the input pixels
        # attribution for the positive prediction (even if the model prediction is negative). So, PPI map can always be 
        # calculated independent of the PPI prediction score by the model.
...
def postproc_attr_result(root_path='./', model_path='./', docking_version='5_5', con_attr_dict_lst=None):
    ...
    for itr in range(start_itr, con_attr_dict_lst_len, 1):
        ...
        # # add postproc_attr_res_dict to postproc_attr_dict_lst
        # postproc_attr_dict_lst.append(postproc_attr_res_dict)
    # end of for loop: for itr in range(start_itr, con_attr_dict_lst_len, 1):
