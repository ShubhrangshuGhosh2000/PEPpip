...
from proc.img_p2ip.img_p2ip_datamodule_train_full_struct import ImgP2ipCustomDataModule
...
class ImgP2ipCnn(L.LightningModule):
    def __init__(self, root_path='./', result_dir='./', model_name='ResNet', config=None):
        ...
        # NOT NEEDED AS TRAINING IS ON FULL TRAINING DATA SET (i.e. Train + Validation) AFTER HYPER-PARAM TUNING
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        self.log("train/train_loss", loss, on_step=False, on_epoch=True, sync_dist=True)
        self.log("train/train_aupr", aupr, on_step=False, on_epoch=True, sync_dist=True)
        # print('training_step :: train/train_loss= ' + str(loss) + ' train/train_aupr= ' + str(aupr))
    ...
    def configure_optimizers(self):
        ...
        # scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer,mode='min', factor=0.1, patience=2, threshold=0.0001, threshold_mode='rel'
        #                                                  , cooldown=0, min_lr=1e-05, eps=1e-08, verbose=True)  # prv_scheduler
        
        scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer,mode='min', factor=0.1, patience=2, threshold=0.01, threshold_mode='rel'
                                                         , cooldown=0, min_lr=1e-05, eps=1e-08, verbose=True)  # modified_scheduler
...
def train_model(root_path='./', result_dir='./', model_name='ResNet', save_name='ResNet', config = None, prv_ckpt_path=None):
    ...
    checkpoint_callback = L.pytorch.callbacks.ModelCheckpoint(
                        ...
                        , filename="ImgP2ipCnn" + "-epoch{epoch:02d}-train_aupr{train/train_aupr:.2f}-last"
    ...
    if(prv_ckpt_path is None):
        ...
        ckpt_file_name = glob.glob(ckpt_path, recursive=False)[-1]
...
def start(root_path='./', model_name = 'ResNet', save_name=None, prv_ckpt_path=None):
    ...
    if(model_name == 'ResNet'):
        ...
        config["img_resoln"] = 400  # img_resoln can be 256 or 400 or 800
        ...
        config["num_workers"] = os.cpu_count() - 5  # for the DataModule creation (in single node case)
...
if __name__ == '__main__':
    ...
    prv_ckpt_path = None
    prv_ckpt_path = os.path.join(root_path, 'dataset/proc_data_tl_feat_to_img/img_p2ip_cnn/train/ResNet_manTlStruct_r400n18D_fullTrain')
    start(root_path=root_path, model_name='ResNet', save_name='ResNet_manTlStruct_r400n18D_fullTrain', prv_ckpt_path=prv_ckpt_path)
