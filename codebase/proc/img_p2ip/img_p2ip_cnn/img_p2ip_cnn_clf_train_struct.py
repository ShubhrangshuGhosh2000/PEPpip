...
from proc.img_p2ip.img_p2ip_datamodule_struct import ImgP2ipCustomDataModule
...
class ImgP2ipCnn(L.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        #                                                  , cooldown=0, min_lr=0, eps=1e-08, verbose=True)  # prv_scheduler
        ...
        #                                                  , cooldown=0, min_lr=1e-04, eps=1e-08, verbose=True)  # modified_scheduler
        ...
        scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer,mode='min', factor=0.1, patience=10, threshold=0.001, threshold_mode='rel'
                                                         , cooldown=0, min_lr=0, eps=1e-08, verbose=True)  # modified_scheduler_v2
...
def train_model(root_path='./', result_dir='./', model_name='ResNet', save_name='ResNet', config = None, prv_ckpt_path=None):
    ...
    if(prv_ckpt_path is None):
        ...
        # ################# TEMP CODE -START ############################################
        # ckpt_file_name = os.path.join(prv_ckpt_path, 'ImgP2ipCnn-epoch17-val_aupr0.90-last.ckpt' )
        # ################# TEMP CODE -END ############################################
...
if __name__ == '__main__':
    ...
    prv_ckpt_path = None
    prv_ckpt_path = os.path.join(root_path, 'dataset/proc_data_tl_feat_to_img/img_p2ip_cnn/train/ResNet_manTlStruct_r400n18D')
    start(root_path=root_path, model_name='ResNet', save_name='ResNet_manTlStruct_r400n18D', prv_ckpt_path=prv_ckpt_path)
