...
def train_model(root_path='./', result_dir='./', model_name='ResNet', save_name='ResNet', config = None, prv_ckpt_path=None):
    ...
    if(prv_ckpt_path is None):
        ...
        # ################# TEMP CODE -START ############################################
        # ckpt_file_name = os.path.join(prv_ckpt_path, 'ImgP2ipCnn-epoch21-val_aupr0.91-last.ckpt' )
        # ################# TEMP CODE -END ############################################
...
if __name__ == '__main__':
    ...
    prv_ckpt_path = None
    prv_ckpt_path = os.path.join(root_path, 'dataset/proc_data_tl_feat_to_img/img_p2ip_cnn/train/ResNet_manTl_r400n18D')
    start(root_path=root_path, model_name='ResNet', save_name='ResNet_manTl_r400n18D', prv_ckpt_path=prv_ckpt_path)
