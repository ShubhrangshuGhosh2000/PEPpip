...
from proc.img_p2ip.img_p2ip_dataset_onlyTl import ImgP2ipCustomDataset
...
class ImgP2ipCustomDataModule(L.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        preproc_data_path = os.path.join(self.root_path, 'dataset/preproc_data_tl_feat_to_img/train_test_list_dump')
        # first retrieve the mean and std of the entire training dataset irrespective of the model training or testing phase
        mean_std_train_df = pd.read_csv(os.path.join(self.root_path, 'dataset/preproc_data_tl_feat_to_img/train_test_list_dump', 'mean_std_train_onlyTl.csv'))
        ...
        if stage == "fit" or stage is None:  # for training
            ...
            oneD_aa_feat_dict_fl_nm_with_path = os.path.join(self.root_path, 'dataset/preproc_data_tl_feat_to_img/oneD_aa_feat', 'oneD_aa_feat_dict_' + self.spec_type + '_len_' + str(self.img_resoln) + '.pkl')
            ...
            # self.train_data = ImgP2ipCustomDataset(twoD_prot_feat_dict, oneD_aa_feat_dict, pp_pair_lst_lsts_train, class_label_lst_train, self.img_resoln, self.transform)
            # self.val_data = ImgP2ipCustomDataset(twoD_prot_feat_dict, oneD_aa_feat_dict, pp_pair_lst_lsts_val, class_label_lst_val, self.img_resoln, self.transform)
            self.train_data = ImgP2ipCustomDataset(root_path=self.root_path, spec_type=self.spec_type, partial_twoD_prot_feat_dict=None, man_2d_feat_dict=None,
                                                   oneD_aa_feat_dict=oneD_aa_feat_dict, pp_pair_lst_lsts=pp_pair_lst_lsts_train, class_label_lst=class_label_lst_train, 
                                                   img_resoln=self.img_resoln, transform=self.transform)
            self.val_data = ImgP2ipCustomDataset(root_path=self.root_path, spec_type=self.spec_type, partial_twoD_prot_feat_dict=None, man_2d_feat_dict=None,
                                                   oneD_aa_feat_dict=oneD_aa_feat_dict, pp_pair_lst_lsts=pp_pair_lst_lsts_val, class_label_lst=class_label_lst_val, 
                                                   img_resoln=self.img_resoln, transform=self.transform)
            ...
            self.test_data = ImgP2ipCustomDataset(root_path=self.root_path, spec_type=self.spec_type, partial_twoD_prot_feat_dict=None, man_2d_feat_dict=None,
