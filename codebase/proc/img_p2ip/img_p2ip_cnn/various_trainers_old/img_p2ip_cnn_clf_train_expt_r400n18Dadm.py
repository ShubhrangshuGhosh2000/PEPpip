...
def train_model(root_path='./', result_dir='./', model_name='ResNet', save_name='ResNet', config = None, prv_ckpt_path=None):
    ...
    early_stop_callback = L.pytorch.callbacks.early_stopping.EarlyStopping(
        ...
        , mode="min"
        , min_delta=0.00, patience=4, verbose=True)  # monitor="val_aupr" can be checked
    ...
    trainer = L.Trainer( default_root_dir=os.path.join(result_dir, save_name)  # Default path for logs and weights when no logger/ckpt_callback passed
                        ...
                        , logger=logger
                        # , logger = False
                        , callbacks=[checkpoint_callback, lr_monitor_callback, early_stop_callback]  # can also use checkpoint_callback, early_stop_callback
...
def start(root_path='./', model_name = 'ResNet', save_name=None, prv_ckpt_path=None):
    ...
    if(model_name == 'ResNet'):
        ...
        config["optimizer_name"] = 'Adam'  # Adam, SGD (FOR RESNET BOTH NEEDS TO BE CHECKED)
...
if __name__ == '__main__':
    ...
    prv_ckpt_path = None
    # prv_ckpt_path = os.path.join(root_path, 'dataset/proc_data_tl_feat_to_img/img_p2ip_cnn/train/ResNet_expt_r400n18D')
    start(root_path=root_path, model_name='ResNet', save_name='ResNet_expt_r400n18Dadm', prv_ckpt_path=prv_ckpt_path)
