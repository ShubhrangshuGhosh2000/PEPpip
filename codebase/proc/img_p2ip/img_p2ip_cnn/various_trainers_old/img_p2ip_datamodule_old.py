...
class ImgP2ipCustomDataModule(L.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        mean_std_train_df = pd.read_csv(os.path.join(self.root_path, 'dataset/preproc_data_tl_feat_to_img/train_test_list_dump', 'mean_std_train.csv'))
        ...
        if stage == "fit" or stage is None:  # for training
            ...
            oneD_prot_feat_dict_fl_nm_with_path = os.path.join(self.root_path, 'dataset/preproc_data_DS', 'DS_seq_feat_dict_prot_t5_xl_uniref50_' + self.spec_type + '.pkl')
            ...
            self.train_data = ImgP2ipCustomDataset(oneD_prot_feat_dict, oneD_aa_feat_dict, pp_pair_lst_lsts_train, class_label_lst_train, self.img_resoln, self.transform)
            self.val_data = ImgP2ipCustomDataset(oneD_prot_feat_dict, oneD_aa_feat_dict, pp_pair_lst_lsts_val, class_label_lst_val, self.img_resoln, self.transform)
            ...
            # create the custom torch dataset to be used by the torch dataloader
            print('creating the custom torch dataset to be used by the torch dataloader')
            self.test_data = ImgP2ipCustomDataset(oneD_prot_feat_dict, oneD_aa_feat_dict, pp_pair_lst_lsts_test, class_label_lst_test, self.img_resoln, self.transform)
