...
class ImgP2ipCnn(L.LightningModule):
    def __init__(self, root_path='./', result_dir='./', model_name='ResNet', config=None):
        ...
        if(self.config["resNet_arch_type"] == 9):
            self.config["c_hidden"] = [16, 32, 64]  
            self.config["num_blocks"] = [3, 3, 3]  
        elif(self.config["resNet_arch_type"] == 18):
            self.config["c_hidden"] = [16, 32, 64, 128, 256]  
            self.config["num_blocks"] = [3, 3, 4, 5, 3]  
        elif(self.config["resNet_arch_type"] == 22):
            self.config["c_hidden"] = [16, 32, 64, 128, 256]  
            self.config["num_blocks"] = [3, 4, 5, 6, 4]  
...
def start(root_path='./', model_name = 'ResNet'):
    ...
    if(model_name == 'ResNet'):
        ...
        config["resNet_arch_type"] = ray.tune.grid_search([9, 18])  
        config["act_fn_name"] = 'relu'  
        config["block_name"] = ray.tune.grid_search(['ResNetBlock'])  
        config["optimizer_name"] = ray.tune.grid_search(['SGD', 'Adam'])  
        config["momentum"] = 0.9  
